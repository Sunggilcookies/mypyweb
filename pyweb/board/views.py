...
# context = {'form': form}
#     return render(request, 'board/question_form.html',context) #get 방식 이거 안쪾으로넣야함
def index(request):
...
def answer_create(request, question_id):
    ...
    question = Question.objects.get(id=question_id)
    if request.method == 'POST': #POST문자열 대문자소문자구분하것
...
#답변 삭제
@login_required(login_url='common:login') #세션없으면 로그인창으로 바로
